...
// the demo.scene test code is here. 
// It does testing sqlitekit against most common functionality which could be used in real-life projects
public class DemoObject1 : MonoBehaviour {
		...
		if( db == null )
		{
			if ( GUI.Button(new Rect (10,10,150,50), "Run SQLite Test \nat persistentDataPath") ) 
			{
				...
				try{
					...
					log += 	"\n on WebGL it must give an exception, it's normal.";
				}
			}
			...
			if ( GUI.Button(new Rect (170,10,150,70), "Run SQLite Test \nfor streamingAssets db\nwhich moved to \npersistentDataPath") ) 
			{
				...
				TestWithStreamingAssetsCopiedToPersistentDataPath();
	...
	// read test.db into byte array
	byte[] ReadTestDatabaseFromStreaminAssets(StringBuilder log) {
		string dbfilename = "test.db";

		byte[] bytes = null;	

#if UNITY_EDITOR || UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX
		string dbpath = "file://" + Application.streamingAssetsPath + "/" + dbfilename; log.Append("test.db path: " + dbpath);
		bytes = ReadFile(dbpath);
#elif UNITY_WEBGL
		string dbpath = "StreamingAssets/" + dbfilename;								log.Append("test.db path: " + dbpath);
		bytes = ReadFile(dbpath);
#elif UNITY_IOS
		string dbpath = Application.dataPath + "/Raw/" + dbfilename;					log.Append("test.db path: " + dbpath);
		try{	
...
#elif UNITY_ANDROID
		string dbpath = Application.streamingAssetsPath + "/" + dbfilename;	            log.Append("test.db path: " + dbpath);
		bytes = ReadFile(dbpath);
#endif
		return bytes;
	}
	...
	#if !SQLITE_NATIVE
	void TestWithStreamingAssets()
	{
		db = new SQLiteDB();

		log = "";

		// buffer to collect log from ReadTestDatabase function
		StringBuilder logOut = new StringBuilder();
		
		byte[] bytes = ReadTestDatabaseFromStreaminAssets(logOut);
		// copy log from ReadTestDatabase.
		log += logOut.ToString();
	...
	void TestWithStreamingAssetsCopiedToPersistentDataPath()
	{
		db = new SQLiteDB();
