...
public class DemoObject4 : MonoBehaviour {
	...
	void CopyFileFromStreamingAssetsToPersistanceFolder(string dbfilename)
	{
		...
		if(!File.Exists(filename))
		{
			...
			if ( bytes != null )
			{
				try{	
					...
					{
                        fs.Write(bytes,0,bytes.Length);             log += "\nCopy database from streaminAssets:" + dbpath + "\nto persistentDataPath: " + filename;
                    }
				...
				log += "no file at: " + dbpath;
			}
		}
	}
        
	void PerformanceTestCoroutine()
	{
		db = new SQLiteDB();
