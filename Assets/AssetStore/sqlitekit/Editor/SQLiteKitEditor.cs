				...
				try{
					...
					if(www!=null)
					{
						if(www.error != null)
						{
							...
							Debug.LogError("Can't download spreadsheet from URL: " + www.error);
							import = false;
						}
						else
						if(www.isDone)
						{
							...
							try{
								...
								List<string> rows = GetCVSLines(www.downloadHandler.text);
								www = null;
								...
								else
								{
									...
									Debug.LogError("Fail to import table '" + table.Name + "' - No columns! Please check source file");
								}
							}
							catch(Exception e)
							{
								...
								Debug.LogError("Fail to import table '" + table.Name + "' with Exception: " + e.Message);
								import = false;
					...
					if( tableIndex < settings.TableCount )
					{
			            ...
			            if (!url.Contains(".google.com"))
			            {
							...
							Debug.LogError("You have entered an incorrect spreadsheet URL. Please read the manuals instructions (See readme.txt)");
			            }
						else{
							Log ("Import table: "+table.Name +" from URL: "+table.Url);
							www = UnityWebRequest.Get(table.Url);
							www.SendWebRequest();
						}
					...
					else
					{
						...
						Debug.Log("All tables was succefully updated!");
					}
    ...
    static List<string> GetCVSLines(string data)
    {
        ...
        while (i < data.Length)
        {
            ...
            else if (data[i] == '\n')
            {
                if (searchCloseTags == 0)
                {
                    lines.Add(data.Substring(lastSentenceStart, i - lastSentenceStart - 1));
