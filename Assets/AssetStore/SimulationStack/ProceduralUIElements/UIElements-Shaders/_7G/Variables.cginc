...
float CalculateSDF(float2 UV)
{
    ...
    _Arc = (_Arc / 180) * (3.1415 / 2.0);
    float SDF = 10.0;
    float SDF0 = length(UV) - _RadiusCenterPoint;
    UV.y -= _RadiusCenterPoint * 0.5;
    float SDF1 = ArcSDF(UV, float2(sin(_Arc), cos(_Arc)), _Separation, _Thickness);
    float SDF2 = ArcSDF(UV, float2(sin(_Arc), cos(_Arc)), _Separation * 2.0, _Thickness);
    float SDF3 = ArcSDF(UV, float2(sin(_Arc), cos(_Arc)), _Separation * 3.0, _Thickness);
    float SDF4 = ArcSDF(UV, float2(sin(_Arc), cos(_Arc)), _Separation * 4.0, _Thickness);
    float SDF5 = ArcSDF(UV, float2(sin(_Arc), cos(_Arc)), _Separation * 5.0, _Thickness);
    SDF = min(SDF, SDF0);
